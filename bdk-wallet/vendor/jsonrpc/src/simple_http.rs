...
impl Default for SimpleHttpTransport {
    fn default() -> Self {
        SimpleHttpTransport {
            ...
            path: "/".to_owned(),
            // PATCHED (mintlayer web-gui): 15s default aborts large `importdescriptors`
            // batches (bdk caches 100 scripts per keychain) on low-end hardware.
            // See bdk-wallet README. Everything else is stock jsonrpc 0.13.0.
